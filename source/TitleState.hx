...
class TitleState extends MusicBeatState
{
	...
	function startIntro()
	{
		...
		logoBl = new FlxSprite(-231, -65);
		logoBl.frames = Paths.getSparrowAtlas('logoBumpin');
		logoBl.antialiasing = ClientPrefs.globalAntialiasing;
		logoBl.animation.addByPrefix('bump', 'ResiseAGAIN', 24);
		...
		titleText = new FlxSprite(745, 160);
		titleText.frames = Paths.getSparrowAtlas('titleEnter');
		titleText.animation.addByPrefix('idle', "Text", 24);
		titleText.animation.addByPrefix('press', "Text", 24);
			...
			if(pressedEnter)
			{
				...
				new FlxTimer().start(1, function(tmr:FlxTimer)
				{
					if (mustUpdate) {
						...
						MusicBeatState.switchState(new LFwarning());
					}
					closedState = true;
				});
