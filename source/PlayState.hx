...
class PlayState extends MusicBeatState
{
	...
	public static var timePlayedFor:Int = 0;
	...
	public var blackGradient:FlxSprite;
	...
	var deCancell:BGSprite;

	var actualGradient:BGSprite;
	...
	var randomXScreen:Int;
	var randomYScreen:Int;
	...
	override public function create()
	{
		...
		switch (curStage)
		{
			...
			case 'housedeleo' : //LeoWeek
				defaultCamZoom = 0.80;
				var bg:BGSprite = new BGSprite('leo/bg', -1050.95, -109.2, 1, 1);
				add(bg);

			case 'bfboi'://Robot leo
				var bg:BGSprite = new BGSprite('leo/bg2', -605.9, -189.6, 1, 1);
				add(bg);

			case 'dread'://Anxiety song
				GameOverSubstate.characterName = 'bfSadDeath';
				var bg:BGSprite = new BGSprite('leo/bg3', -2885.65, -706.2, 1, 1);
				add(bg);
				bg.screenCenter();

				var deGradient:BGSprite = new BGSprite('leo/Gradient', 982.9, 463.45, 1, 1);
				add(deGradient);
				deGradient.screenCenter();
				deGradient.cameras = [camHUD];

				deCancell = new BGSprite('leo/cancelled', 0, 0, 1, 1);
				add(deCancell);
				deCancell.screenCenter();
				deCancell.cameras = [camOther];
				deCancell.alpha = 0;
		...
		{
			dad.setPosition(297.15, 165.35);
			boyfriend.setPosition(1165, 461.05);
			gf.setPosition(572.95, 45.75);
		}

		if(curStage == 'bfboi')
		{
			dad.setPosition(157.25, 341.35);
			boyfriend.setPosition(1157.65, 518);
			gf.setPosition(525.65, -13.95);
		}

		if(curStage == 'dread')
		{
			dad.setPosition(0, 60.35);
			boyfriend.setPosition(1030.95, 393.65);
			gf.setPosition(0, 10000);
		}

		if(dad.curCharacter == 'anxiety')
		{
			dad.playAnim('begin1', true);
	...
	override public function update(elapsed:Float)
	{
		...
		if(curStage == 'dread')
		{
			...
			{
				deCancell.alpha -= 0.00125;
			}

			if(deCancell.alpha >= 0.90)
			{
				health = 0;
				doDeathCheck();
			}
		...
		//FlxTween.tween(camFollowPos, {x: 200, y: 500}, 2, {ease: FlxEase.expoOut});
		...
		FlxG.watch.addQuick("camX", camFollowPos.x);
		FlxG.watch.addQuick("camY", camFollowPos.y);
	...
	function goodNoteHit(note:Note):Void
	{
		...
		if(note.noteType == 'Troll Note')
		{
			...
			randomXScreen = FlxG.random.int(0, 1920);
			randomYScreen = FlxG.random.int(0, 1080);
			Lib.application.window.resize(randomXScreen, randomYScreen);
			//leo u prankster :troll:
				
		}
		
		if(note.noteType == 'Vent Note')
		{
			deCancell.alpha += 0.25;
			//camGame.shake(1, 0.5);
			//camHUD.shake(1, 0.5);
